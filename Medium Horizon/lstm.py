...
class CustomLSTMCell(nn.Module):
    ...
    def forward(self, x_t, h_prev, c_prev):
        ...
        f_t = torch.sigmoid(f_t) # all sizes (bath, hidden_size)
        ...
        return h_t, c_t # outputs here are (batch_size, hidden_size)
...
class CustomLSTMForecast(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc(h_t)  # use last hidden state (batch, 1)
        return out
    
# input_size = number of features per time step
# hidden_size = the size of the vector of the h_t and c_t
# seq_len = the number of time steps considered for prediction