...
for file in excel_files:
    try:
        ...
        for iterations in iterations_list:
            for depth in depth_list:
                ...
                if val_loss < best_val_loss:
                    ...
                    best_config = {'iterations': iterations, 'depth': depth}
                    best_model = model
        ...
        for i in range(len(test_series)):
            ...
            preds.append(pred)
            context.append(pred)  # Use predicted value for next input
...
df.to_csv('CatBoost_sMAPE.csv', index=False)

with open('catboost actuals.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    for file_name, row in zip(file_names_evaluated, all_actuals_combined):
        writer.writerow([file_name] + row.flatten().tolist())

with open('catboost preds.csv', 'w', newline='') as f:
    writer = csv.writer(f)
