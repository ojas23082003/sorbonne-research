...
for file in excel_files:
    try:
        ...
        all_actuals_combined.append(all_actuals)
...
df.to_csv('Chronos t5 tiny sMAPE.csv', index=False)

with open('chronos actuals.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    for file_name, row in zip(file_names_evaluated, all_actuals_combined):
        writer.writerow([file_name] + row.flatten().tolist())

with open('chronos tiny preds.csv', 'w', newline='') as f:
    writer = csv.writer(f)
